...
func (h *OpenAIHandler) forward(w http.ResponseWriter, r *http.Request, et domain.EndpointType) {
	...
	if position == -1 {
		metrics.Default.IncRequests(modelAlias, "queue_full")
		writeError(w, http.StatusServiceUnavailable, "queue is full")
		return
	}
	metrics.Default.SetQueueLength(modelAlias, length)
	...
	case result := <-pending.ResultChan:
		...
		metrics.Default.IncRequests(modelAlias, statusStr)
		...
		if totalTokens > 0 {
			metrics.Default.AddTokens(modelAlias, "prompt", promptTokens)
			metrics.Default.AddTokens(modelAlias, "completion", completionTokens)
			go func() {
	...
	case <-reqCtx.Done():
		if errors.Is(reqCtx.Err(), context.DeadlineExceeded) {
			metrics.Default.IncRequests(modelAlias, "timeout")
			// Queue wait timeout
			...
			metrics.Default.IncRequests(modelAlias, "cancelled")
			// Client disconnected
