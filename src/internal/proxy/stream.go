...
func ForwardStream(ctx context.Context, w http.ResponseWriter, resp *http.Response) (*StreamResult, error) {
	...
	if !usageFound {
		...
		// Use rune count for a better estimate: ~1.5 tokens per CJK character,
		// ~0.25 tokens per ASCII character (≈4 chars/token).
		// Averaging across mixed content, rune count / 2 is a reasonable middle ground.
		runes := len([]rune(content))
		if runes == 0 {
			result.CompletionTokens = 0
		} else {
			result.CompletionTokens = max(1, runes/2)
		}
	}
